...
app.use((req, res, next) => {
  ...
  // This avoids hangs or timeouts on other requests on serverless infrastructure.
...
app.post(['/api/auth/otp/send', '/auth/otp/send'], async (req, res) => {
  try {
          ...
          html: `
            <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; max-width: 500px; margin: 0 auto; padding: 24px; border: 1px solid #e2e8f0; border-radius: 16px; background-color: #fafafa;">
              ...
              <p style="color: #64748b; font-size: 12px; line-height: 1.5; text-align: center; margin-bottom: 0;">
                Ce code is strictement unique et confidentiel. Il est valable pendant 10 minutes.<br>Si vous n'êtes pas à l'origine de cette demande, vous pouvez ignorer cet e-mail en toute sécurité.
              </p>
            </div>
          `,
        }),
    ...
    const stored = emailVal ? otpStorage.get(emailVal) : null;
...
async function bootstrap() {
  ...
  if (process.env.VERCEL) {
    console.log(
      'Environnement Vercel détecté : initialisation sans app.listen ni fichiers statiques.',
    );
    return;
  }
  ...
  if (process.env.NODE_ENV !== 'production') {
    ...
    // Production Mode: dynamically check if running inside "dist" or if dist directory contents are in cwd (www)
