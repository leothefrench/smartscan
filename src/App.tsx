...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsubPremium = onSnapshot(doc(db, "users", userId), (docSnap) => {
      ...
      setFirestoreConnected(false);
      setSyncError(`Erreur Sync Premium : ${err.message || "Permissions insuffisantes"}`);
    });
    ...
    const unsubReceipts = onSnapshot(collection(db, "users", userId, "receipts"), (qSnapshot) => {
      ...
      setFirestoreConnected(false);
      setSyncError(`Erreur Sync Tickets : ${err.message || "Permissions insuffisantes"}`);
    });
  ...
  return (
    <div className="min-h-screen bg-black text-zinc-150 flex flex-col font-sans selection:bg-emerald-500/30 selection:text-emerald-400" id="app-root">
      ...
      <main className="flex-1 w-full max-w-5xl mx-auto px-4 py-8 space-y-6">
        ...
        <div className="p-6 bg-zinc-950 border border-zinc-900 rounded-3xl space-y-4 text-left" id="diagnostics-panel">
          <div className="flex items-center gap-2">
            <AlertTriangle size={18} className="text-amber-500 shrink-0" />
            <h3 className="text-sm font-bold text-white tracking-tight">Zone de diagnostic & synchronisation</h3>
          </div>
          <p className="text-xs text-zinc-400 leading-relaxed">
            Si vos appareils (PC et Mobile) n'affichent pas le même nombre de tickets ou le même statut PRO alors que vous êtes connectés, vérifiez que l'adresse e-mail affichée est strictement identique (au caractère près, sans espace ni faute de frappe).
          </p>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs">
            <div className="bg-zinc-900/60 p-3 rounded-xl border border-zinc-800 space-y-1">
              <span className="text-[10px] text-zinc-500 font-mono block">E-MAIL ACTUELLEMENT CONNECTÉ :</span>
              <span className="font-mono text-white font-semibold break-all">{currentUserEmail || "Non connecté"}</span>
            </div>
            <div className="bg-zinc-900/60 p-3 rounded-xl border border-zinc-800 space-y-1">
              <span className="text-[10px] text-zinc-500 font-mono block">IDENTIFIANT UNIQUE DU CLOUD :</span>
              <span className="font-mono text-emerald-400 font-semibold break-all">
                {currentUserEmail ? currentUserEmail.toLowerCase().replace(/[^a-zA-Z0-9_\-]/g, "_") : "aucun"}
              </span>
            </div>
          </div>
          ...
          <div className="bg-zinc-900/40 p-3.5 rounded-xl border border-zinc-850/50 space-y-2">
            <p className="text-[11px] text-zinc-400 leading-relaxed">
              <strong>💡 Rappel technique :</strong> Le stockage local de secours de votre téléphone est physique et indépendant de celui de votre ordinateur. Seule la connexion au Cloud Firestore unifie vos appareils. Si vous souhaitez réinitialiser tout le cache local et vous reconnecter proprement, cliquez sur le bouton ci-dessous :
            </p>
            <button
              type="button"
              onClick={() => {
                if (window.confirm("Voulez-vous vraiment vider le cache local de cet appareil et vous déconnecter ? Les données non synchronisées avec le Cloud seront supprimées de cet appareil.")) {
