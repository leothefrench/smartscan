...
export default function App() {
  ...
  useEffect(() => {
    ...
    let autologinEmail = params.get('autologin');
  ...
  useEffect(() => {
    ...
    const triggerFastHttpSync = async () => {
      try {
        ...
        setSyncError(null);
        console.log(
          '[SmartReceipt REST] Synchronisation initiale ultra-rapide réussie.',
        );
      } catch (err: any) {
        console.warn(
          "[SmartReceipt REST Sync Warning] Échec de la synchronisation REST initiale, le SDK d'arrière-plan prend le relais:",
          err,
        );
      }
    };
  ...
  const handleLoginSuccess = async (email: string) => {
    ...
    setIsAuthenticated(true);
    localStorage.setItem('scanner_user_session', cleanEmail);
