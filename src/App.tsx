...
import { Scan, Sparkles, ReceiptText, ShieldCheck, LogOut, User, Cloud, CloudOff, QrCode, Smartphone, X } from "lucide-react";
...
export default function App() {
  ...
  const handleScanSuccess = (data: any, originalImageName: string, base64Preview?: string) => {
    ...
    <div className="min-h-screen bg-black text-zinc-150 flex flex-col font-sans selection:bg-emerald-500/30 selection:text-emerald-400" id="app-root">
      ...
      <header className="sticky top-0 z-40 bg-zinc-950/80 backdrop-blur-md border-b border-zinc-900 px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-sm font-bold text-white tracking-tight flex items-center gap-1.5 leading-none">
              SmartScan <span className="text-[9px] bg-zinc-900 text-emerald-400 px-1.5 py-0.5 rounded font-mono font-medium border border-zinc-800">PRO-SECURE</span>
            </h1>
            <span className="text-[10px] text-zinc-400 font-medium">Gestionnaire intelligent & ultra-privé de tickets</span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          {IS_FIREBASE_REAL ? (
            <div className="flex items-center gap-1.5 text-xs text-emerald-400 font-semibold bg-emerald-950/30 border border-emerald-900/50 px-3 py-1.5 rounded-full" title="Base de données Cloud active">
              <Cloud size={14} className="animate-pulse text-emerald-400" />
              Cloud Synced
            </div>
          ) : (
            <div className="flex items-center gap-1.5 text-xs text-zinc-400 font-semibold bg-zinc-900 border border-zinc-800 px-3 py-1.5 rounded-full" title="Sauvegarde locale chiffrée. Connectez Firebase pour synchroniser.">
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-6 max-w-sm w-full relative space-y-4 shadow-2xl">
            ...
            <div className="space-y-2 text-[11px] text-zinc-350">
              <div className="flex items-start gap-1.5">
                ...
                <span>Ouvrez l'application <strong>Appareil Photo</strong> or le raccourci <strong>Scanner</strong> de votre Xiaomi.</span>
              </div>
              <div className="flex items-start gap-1.5">
