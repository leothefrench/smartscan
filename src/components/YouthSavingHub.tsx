...
export default function YouthSavingHub({
  ...
  const fastFoodSpend = receipts.reduce((sum, r) => {
    const isFastFoodMerchant = fastFoodKeywords.some((kw) =>
    ...
    const itemSpend = r.items.reduce((itemSum, item) => {
      const isFastFoodItem = fastFoodKeywords.some((kw) =>
  ...
  const progressPercent = Math.min(
    (fastFoodSpend / fastFoodThreshold) * 100,
    100,
  );

  // 2. Interactive simulator states
  const [coffeeCount, setCoffeeCount] = useState(4); // coffees per week
  const [coffeePrice, setCoffeePrice] = useState(2.2); // coffee price in euros (e.g. 1.5 province, 2.2 moyenne, 3 Paris)
  ...
  const weeklyCoffeeCost = coffeeCount * coffeePrice;
  ...
  let rankDescription =
    'Commencez à scanner vos tickets de caisse pour débloquer votre statut et optimiser votre budget.';
  ...
  if (scanCount >= 8) {
    ...
    rankDescription =
      "Exceptionnel ! Vous analysez systématiquement chaque flux pour préserver votre capital d'investissement.";
  } else if (scanCount >= 4) {
    userRank = "Ninja de l'Épargne 🥷";
    rankColor = 'text-cyan-400 bg-cyan-950/40 border border-cyan-500/30';
    rankDescription =
      'Excellent rythme. Vous automatisez la traque de la TVA et de vos charges courantes.';
  } else if (scanCount > 0) {
    userRank = 'Économe Actif ⚡';
    rankColor = 'text-amber-400 bg-amber-950/40 border border-amber-500/30';
    rankDescription =
      'Bon début ! Continuez à enregistrer vos reçus pour dresser un historique et optimiser vos achats.';
  }
  ...
  return (
    <div className="space-y-6" id="youth-saving-hub">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div
          className="bg-gradient-to-br from-zinc-900 to-zinc-950/80 rounded-2xl p-6 border border-zinc-700 flex flex-col justify-between relative overflow-hidden"
          id="challenge-card"
        >
            ...
            <div className="flex items-center gap-2 mb-4">
              ...
              <div>
                ...
                <span className="text-[10px] text-zinc-400">
                  Objectif mensuel maximal conseillé : {fastFoodThreshold} €
                </span>
              </div>
            </div>

            <p className="text-xs text-zinc-300 leading-relaxed mb-6">
              Le budget restauration extérieure (fast-foods, cafés,
              boulangeries, livraisons à domicile) est le premier pôle
              d'optimisation financière chez les particuliers souhaitant
              ...
              <div className="flex justify-between text-[10px] text-zinc-500 font-medium">
                ...
                {progressPercent >= 100 ? (
                  <span className="text-red-400 font-semibold">
                    🚨 Seuil d'alerte dépassé - Privilégiez le fait-maison !
                  </span>
                ) : progressPercent >= 80 ? (
        ...
        >
          <div>
            <div className="flex items-center gap-2 mb-4">
              ...
              <div>
                ...
                <span className="text-[10px] text-zinc-400">
                  Mesurez l'impact des dépenses récurrentes
                </span>
              </div>
            </div>
              ...
              <div className="space-y-1.5">
                <div className="flex justify-between text-xs font-semibold text-zinc-300">
                  <span className="flex items-center gap-1">
                    <Coffee size={13} className="text-zinc-500" /> Fréquence
                    (Cafés / Encas)
                  </span>
                ...
                {/* Customizable Coffee Unit Price Sub-Section */}
                <div className="bg-zinc-950/40 p-2.5 rounded-xl border border-zinc-800/60 space-y-2 mt-1">
                  <div className="flex justify-between items-center text-[10px] font-bold text-zinc-450 uppercase tracking-wider">
                    <span className="text-zinc-400">Tarif unitaire estimé</span>
                  ...
                  <div className="grid grid-cols-3 gap-1">
                    <button
                      ...
                      title="Tarif provincial (ex: Normandie, Bretagne)"
                    >
                      Normandie (1.50€)
                    </button>
                    <button
                      type="button"
                      onClick={() => setCoffeePrice(2.2)}
                      className={`py-1 text-[9px] font-black rounded-lg transition-all border cursor-pointer ${
                        Math.abs(coffeePrice - 2.2) < 0.05
                          ? 'bg-amber-500/10 border-amber-500/40 text-amber-400 font-extrabold'
                          : 'bg-zinc-900 border-zinc-800 text-zinc-450 hover:text-zinc-300'
                      }`}
                      title="Tarif moyen national"
                    >
                      Moyenne (2.20€)
                    </button>
                    <button
                      type="button"
                      onClick={() => setCoffeePrice(3.0)}
                      className={`py-1 text-[9px] font-black rounded-lg transition-all border cursor-pointer ${
                        Math.abs(coffeePrice - 3.0) < 0.05
                          ? 'bg-amber-500/10 border-amber-500/40 text-amber-400 font-extrabold'
                          : 'bg-zinc-900 border-zinc-800 text-zinc-450 hover:text-zinc-300'
                      }`}
                      title="Tarif de grande métropole (ex: Paris centre, Lyon)"
                    >
                      Paris (3.00€)
                    </button>
                  </div>
                  ...
                  <div className="flex items-center gap-2 pt-0.5">
                    ...
                    <input
                      ...
                      className="flex-1 h-1 bg-zinc-900 rounded appearance-none cursor-pointer accent-amber-500"
                    />
                  </div>
                </div>
              </div>
          ...
          <div className="mt-5 pt-3.5 border-t border-zinc-800 space-y-1.5 font-sans">
            ...
            <p className="text-[9px] text-zinc-500 leading-normal italic text-center pt-2 select-none border-t border-dashed border-zinc-800">
              💡 Placer cette somme sur un plan d'épargne permet d'accélérer la
              ...
              <div>
                <div className="flex items-center gap-2 text-amber-400 mb-2">
                  ...
                  <h3 className="text-xs font-black tracking-wider uppercase">
                    Attention Rétractation
                  </h3>
                </div>
                <p className="text-[11px] text-zinc-300 leading-normal mb-3">
                  En renonçant à votre abonnement Premium PRO, vous abandonnez
                  les engagements de qualité qui y sont attachés :
                </p>
                ...
                <div className="space-y-2 bg-zinc-950/60 p-3 rounded-xl border border-zinc-900/80">
                  <div className="flex items-start gap-1.5 text-[10px] text-zinc-400">
                    ...
                    <span>
                      <strong>Analyses & Scans Illimités</strong> (limitation à
                      un seuil d'essai gratuit)
                    </span>
                  </div>
                  <div className="flex items-start gap-1.5 text-[10px] text-zinc-400">
                    <span className="text-rose-500 font-bold shrink-0">✕</span>
                    <span>
                      <strong>Exportation Comptable complète</strong> (les
                      exports Excel/CSV seront verrouillés)
                    </span>
                  </div>
                  <div className="flex items-start gap-1.5 text-[10px] text-zinc-400">
                    <span className="text-rose-500 font-bold shrink-0">✕</span>
                    <span>
                      <strong>Abonnements Fantômes</strong> (traque et
                      notification automatique suspendues)
                    </span>
                  </div>
                  <div className="flex items-start gap-1.5 text-[10px] text-zinc-400">
                    <span className="text-rose-500 font-bold shrink-0">✕</span>
                    <span>
                      <strong>Synchronisation Cloud instantanée</strong> sur
                      tous vos écrans inactive
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-3.5">
                  <div>
                    <h3 className="text-sm font-extrabold text-white tracking-tight flex items-center gap-1.5">
                      SmartScan Premium{' '}
                      <span className="text-[10px] text-amber-400 bg-amber-950/80 px-1.5 py-0.5 rounded border border-amber-500/20 uppercase tracking-widest">
                        PRO
                      </span>
                    </h3>
                    <p className="text-[10px] text-zinc-400 font-medium">
                      Maximisez votre gestion financière
                    </p>
                  </div>
                </div>
                ...
                <div className="space-y-2 text-[11px] text-zinc-300">
                  <div className="flex items-start gap-1.5">
                    ...
                    />
                    <span>
                      <strong>Analyses & Scans Illimités</strong> (Analyse en
                      temps réel de tous vos reçus)
                    </span>
                  </div>
                  <div className="flex items-start gap-1.5">
                    <Check
                    ...
                    />
                    <span>
                      <strong>Exportation Comptable Complète</strong> (Fichiers
                      Excel ou CSV avec la TVA détaillée)
                    </span>
                  </div>
                  <div className="flex items-start gap-1.5">
                    <Check
                    ...
                    />
                    <span>
                      <strong>Abonnements Fantômes</strong> (Détection
                      intelligente de frais récurrents cachés)
                    </span>
                  </div>
                  <div className="flex items-start gap-1.5">
                    <Check
                    ...
                    />
                    <span>
                      <strong>Multi-Appareils Cloud</strong> (Accès et
                      sauvegarde sécurisés instantanés)
                    </span>
                  </div>
                </div>
              </div>
      ...
      <div
        className="bg-zinc-900/60 p-5 border border-zinc-800/80 rounded-2xl flex flex-col sm:flex-row items-center sm:justify-between gap-4"
        id="loyalty-tier-card"
      >
        <div className="flex items-center gap-3.5 text-center sm:text-left flex-col sm:flex-row">
          <div
            className={`p-3.5 rounded-full flex items-center justify-center shrink-0 ${rankColor}`}
          >
            <Award size={24} />
          </div>
          <div className="space-y-1">
            <div className="flex items-center justify-center sm:justify-start gap-2 flex-wrap">
              ...
              <span
                className={`text-[10px] font-extrabold px-2 py-0.5 rounded-full uppercase ${rankColor}`}
              >
                {userRank}
              </span>
            </div>
        ...
        <div className="text-center sm:text-right shrink-0">
          <div className="text-[10px] text-zinc-500 uppercase tracking-widest font-bold">
            Progression de scans
          </div>
          <div className="text-lg font-black text-white font-mono mt-0.5">
            {scanCount} / 8 scannés
          </div>
          <p className="text-[9px] text-zinc-500 mt-0.5">
            Rang maximal (Champion) à 8 scans
          </p>
        </div>
      </div>
    </div>
