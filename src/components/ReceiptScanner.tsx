...
export default function ReceiptScanner({ onScanSuccess, isPremium = false }: ReceiptScannerProps) {
  ...
  const compressImageClientSide = (file: File, maxWidth = 720, maxHeight = 720, quality = 0.65): Promise<{ base64Data: string; fullBase64: string; mimeType: string }> => {
  ...
  const handleManualSubmit = (e: React.FormEvent) => {
    ...
    const manualReceipt: Receipt = {
      ...
      rawResponse: isRecurring 
        ? `Prélèvement ou abonnement périodique récurrent (${recurrenceInterval === "weekly" ? "Hebdomadaire" : recurrenceInterval === "monthly" ? "Mensuel" : "Annuel"}).\nLibellé : ${sanitizedName}\nFrais fixes récurrents.`
  ...
  return (
    ...
    >
      ...
      {activeTab === "scan" ? (
        ...
        /* Manual Quick Form */
        <form onSubmit={handleManualSubmit} className="bg-zinc-950/80 rounded-2xl p-5 border border-zinc-700 space-y-4 font-sans" id="manual-expense-form">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="md:col-span-2 border-t border-zinc-900 pt-3 mt-1 space-y-2.5">
              <label className="flex items-start gap-2.5 text-xs text-zinc-300 font-semibold cursor-pointer select-none">
                ...
                />
                <div>
                  <span className="text-white font-bold block">Abonnement ou Prélèvement récurrent</span>
                  <span className="text-[11px] text-zinc-500 font-normal">Cochez cette option s'il s'agit d'une charge fixe périodique (Netflix, Spotify, salle de sport, électricité, loyer...)</span>
                </div>
              </label>
