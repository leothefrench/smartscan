...
export default function StatsOverview({ receipts }: StatsOverviewProps) {
  ...
  const sortedCategories = (Object.keys(CATEGORY_COLORS) as ReceiptCategory[])
    .map((cat) => ({
      ...
      percentage: totalSpent > 0 ? ((categorySums[cat] || 0) / totalSpent) * 100 : 0
    }))
    .sort((a, b) => b.amount - a.amount);

  const topCategory = sortedCategories[0]?.amount > 0 ? sortedCategories[0] : null;

  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8" id="stats-container">
      ...
      >
        <div className="flex items-center justify-between">
          <span className="text-xs font-semibold uppercase tracking-wider text-zinc-400">Dépenses Totales</span>
        ...
        <div className="my-4">
          ...
          <p className="text-xs text-zinc-400 mt-1">
            Basé sur <span className="font-semibold text-zinc-200">{totalCount} ticket{totalCount > 1 ? 's' : ''}</span> numérisé{totalCount > 1 ? 's' : ''}
          </p>
        </div>
        <div className="pt-4 border-t border-zinc-850 flex items-center justify-between text-xs text-zinc-400">
          ...
          <span className="font-bold text-white">
            {averageSpent.toLocaleString("fr-FR", { style: "currency", currency: "EUR" })}
          </span>
        </div>
      </div>
      ...
      >
        <div className="flex items-center justify-between">
          <span className="text-xs font-semibold uppercase tracking-wider text-zinc-400">Poste Principal</span>
          ...
          {topCategory ? (
            <>
              ...
              <div className="text-base font-semibold text-emerald-400 mt-0.5">
                {topCategory.amount.toLocaleString("fr-FR", { style: "currency", currency: "EUR" })}
            ...
            <>
              <div className="text-lg font-medium text-zinc-500 italic">Aucun ticket encore scanné</div>
              <div className="text-xs text-zinc-500 mt-2">Glissez un ticket d'exemple pour tester.</div>
            </>
          )}
        </div>
        <div className="pt-4 border-t border-zinc-850 flex items-center justify-between text-xs text-zinc-400">
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <span className="text-xs font-semibold uppercase tracking-wider text-zinc-400">Répartition des Coûts</span>
          <div className="p-2 bg-zinc-800/80 rounded-lg text-zinc-350 border border-zinc-850">
            <PieChart size={16} />
          </div>
        </div>
        <div className="space-y-3 max-h-[140px] overflow-y-auto pr-1">
          {totalSpent > 0 ? (
            sortedCategories
              .filter((c) => c.amount > 0)
              .map((cat) => {
                const colors = CATEGORY_COLORS[cat.name] || CATEGORY_COLORS["Autre"];
                ...
                if (catStr.includes("Alimentation")) customBg = "bg-emerald-500";
                ...
                else if (catStr.includes("Services")) customBg = "bg-indigo-505";
            ...
            <div className="flex flex-col items-center justify-center py-6 text-center">
              <span className="text-xs text-zinc-500 italic">Données de répartition inutilisées</span>
            </div>
          )}
        </div>
      </div>
