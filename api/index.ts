...
try {
  ...
  for (const p of pathsToTry) {
    if (fs.existsSync(p)) {
      ...
      console.log(
        '[SmartReceipt] Configuration Firebase REST chargée dynamiquement depuis :',
        p,
      );
      loaded = true;
      break;
  ...
  if (!loaded) {
    console.log(
      '[SmartReceipt] Fichier de configuration Firebase introuvable au runtime. Utilisation des constantes de secours.',
    );
  }
} catch (err) {
  console.warn(
    '[SmartReceipt] Échec du chargement dynamique de la configuration Firebase. Utilisation des constantes de secours :',
    err,
  );
}

console.log(
  '[SmartReceipt] Firebase REST configuration loaded. Project ID:',
  firebaseProjectId,
);
...
async function saveOTP(emailKey: string, code: string, expiresAt: number) {
  ...
  if (firebaseProjectId) {
    try {
      const url = `https://firestore.googleapis.com/v1/projects/${firebaseProjectId}/databases/(default)/documents/otps/${encodeURIComponent(
        emailKey,
      )}?key=${firebaseApiKey}`;
      ...
      if (response.ok) {
        console.log(
          `[SmartReceipt] Saved OTP for ${emailKey} in Firestore REST.`,
        );
      } else {
        const errorText = await response.text();
        console.warn(
          '[SmartReceipt] Firestore REST write response error:',
          errorText,
        );
      }
    } catch (dbErr) {
      console.warn(
        '[SmartReceipt] Failed to write OTP to Firestore REST, falling back to local memory:',
        dbErr,
      );
    }
  }
...
async function getOTP(
  ...
  if (firebaseProjectId) {
    try {
      ...
      if (response.ok) {
        ...
        const errorText = await response.text();
        console.warn(
          '[SmartReceipt] Firestore REST read response error:',
          errorText,
        );
      }
    } catch (dbErr) {
      console.warn(
        '[SmartReceipt] Failed to read OTP from Firestore REST, trying local memory:',
        dbErr,
      );
    }
  }
  ...
  if (local && Date.now() < local.expiresAt) {
    console.log(
      `[SmartReceipt] Retrieved active OTP for ${emailKey} from local memory.`,
    );
    return local;
  }
  return null;
...
async function removeOTP(emailKey: string) {
  if (firebaseProjectId) {
    try {
      ...
      console.log(
        `[SmartReceipt] Deleted OTP for ${emailKey} from Firestore REST.`,
      );
    } catch (dbErr) {
      console.warn(
        '[SmartReceipt] Failed to delete OTP from Firestore REST:',
        dbErr,
      );
    }
  }
      ...
      config: {
        ...
        responseSchema: {
          ...
          properties: {
            ...
            date: {
              type: Type.STRING,
              description: `La date du ticket au format YYYY-MM-DD. Si absente, extrais l'année/mois puis estime ou utilise la date courante du jour de l'analyse : ${
            ...
            rawResponse: {
              type: Type.STRING,
              description:
                "Une note d'analyse budgétaire courte, lucide et critique en français (15-25 mots). Pas de compliments futiles ou de flatteries. Identifie un conseil d'économie concret ou une alternative fait maison (ex: cuisiner soi-même par rapport à un plat acheté préparé ou traiteur) pour optimiser les dépenses de ce ticket.",
            },
          },
        },
      },
...
function getStripe(): Stripe {
  if (!stripeClient) {
    ...
    if (!key) {
      throw new Error(
        "La clé d'API STRIPE_SECRET_KEY est requise ou manquante dans .env.",
      );
    }
...
async function setPremiumStatusRest(
  userId: string,
  isPremium: boolean,
): Promise<void> {
  if (!firebaseProjectId) {
    throw new Error(
      'Aucun projet Firebase configuré pour la mise à jour REST Premium.',
    );
  }
  ...
  const url = `https://firestore.googleapis.com/v1/projects/${firebaseProjectId}/databases/(default)/documents/users/${encodeURIComponent(
    emailKey,
  )}?key=${firebaseApiKey}`;
  const response = await fetch(url, {
    method: 'PATCH',
  ...
  if (response.ok) {
    console.log(
      `[Stripe Webhook] Firestore Premium activé avec succès pour ${emailKey} (${isPremium})`,
    );
  } else {
...
app.post(
  ['/api/stripe/create-checkout-session', '/stripe/create-checkout-session'],
  async (req, res) => {
    try {
      ...
      if (!email || !userId) {
        res
          ...
          .json({
            success: false,
            error:
              "L'e-mail de l'abonné et l'identifiant de son compte sont requis.",
          });
        return;
      }
      ...
      if (
        !rawUrl ||
        rawUrl.toUpperCase().includes('MY_APP_URL') ||
        rawUrl.toUpperCase().includes('PLACEHOLDER')
      ) {
        rawUrl =
      ...
      if (!appUrl.startsWith('http://') && !appUrl.startsWith('https://')) {
        ...
        if (
          ...
          appUrl.startsWith('192.') ||
          appUrl.startsWith('10.')
        ) {
          appUrl = `http://${appUrl}`;
        } else {
          appUrl = `https://${appUrl}`;
      ...
      console.log(
        `[Stripe] Création d'une session de souscription Checkout pour ${cleanUserId} (${cleanEmail})`,
      );
        ...
        line_items: [
          {
            price_data: {
              ...
              product_data: {
                ...
                description:
                  'Abonnement Mensuel SmartScan - Scans illimités, exports illimités et IA prioritaire.',
              },
      ...
      console.error(
        '[Stripe] Échec de la création de la session Checkout :',
        error,
      );
      res
        .status(500)
        .json({
          success: false,
          error:
            error.message || 'Erreur lors de la communication avec Stripe.',
        });
    }
  },
);
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (webhookSecret && stripeSignature) {
        ...
        console.log(
          `[Stripe Webhook] Webhook vérifié avec succès. Événement : ${stripeEvent.type}`,
        );
      } else {
        ...
        console.log(
          `[Stripe Webhook Warning] Exécution sans clé de validation de signature. Événement : ${stripeEvent?.type}`,
        );
      }
    } catch (err: any) {
      console.error(
        `[Stripe Webhook Error] Échec de validation du Webhook :`,
        err.message,
      );
    ...
    try {
      if (stripeEvent.type === 'checkout.session.completed') {
        ...
        if (userId) {
          console.log(
            `[Stripe Webhook] Commande d'abonnement clôturée avec succès. Activation PRO pour : ${userId}`,
          );
        ...
        if (userId) {
          if (
            ...
            console.log(
              `[Stripe Webhook] Statut d'abonnement inactif/résilié (${status}). Désactivation PRO pour : ${userId}`,
            );
            await setPremiumStatusRest(userId, false);
          } else if (status === 'active') {
            console.log(
              `[Stripe Webhook] Statut d'abonnement actif. Réactivation/Maintien PRO pour : ${userId}`,
            );
            await setPremiumStatusRest(userId, true);
          }
        }
      ...
      console.error(
        `[Stripe Webhook] Exception lors du traitement de l'événement de webhook :`,
        processErr,
      );
    }
...
app.post('/api/users/:userId/receipts', async (req, res) => {
  try {
    ...
    const url = `https://firestore.googleapis.com/v1/projects/${firebaseProjectId}/databases/(default)/documents/users/${encodeURIComponent(
      cleanUserId,
    )}/receipts/${receipt.id}?key=${firebaseApiKey}`;
    const body = {
...
app.delete('/api/users/:userId/receipts/:receiptId', async (req, res) => {
  try {
    ...
    const url = `https://firestore.googleapis.com/v1/projects/${firebaseProjectId}/databases/(default)/documents/users/${encodeURIComponent(
      userId,
    )}/receipts/${receiptId}?key=${firebaseApiKey}`;
    const response = await fetch(url, {
      method: 'DELETE',
    });
    if (response.ok) {
...
app.post('/api/users/:userId/receipts/bulk-sync', async (req, res) => {
  try {
    ...
    const cloudUrl = `https://firestore.googleapis.com/v1/projects/${firebaseProjectId}/databases/(default)/documents/users/${encodeURIComponent(
      cleanUserId,
    )}/receipts?key=${firebaseApiKey}`;
    ...
    const missingReceipts = receipts.filter(
      (r: any) => r && r.id && !cloudIds.has(r.id),
    );
    if (missingReceipts.length > 0) {
      const uploadPromises = missingReceipts.map(async (local: any) => {
        const url = `https://firestore.googleapis.com/v1/projects/${firebaseProjectId}/databases/(default)/documents/users/${encodeURIComponent(
          cleanUserId,
        )}/receipts/${local.id}?key=${firebaseApiKey}`;
        const body = {
