...
try {
  ...
  if (fs.existsSync(configPath)) {
    ...
    console.log(
      '[SmartReceipt] Firebase REST configuration loaded successfully. Project ID:',
      firebaseProjectId,
    );
  } else {
    console.warn(
...
async function setPremiumStatusRest(
  ...
  if (!firebaseProjectId) {
    console.warn(
      '[Stripe Webhook] Aucun projet Firebase configuré pour la mise à jour REST Premium.',
    );
    return;
  }
  ...
  try {
    ...
    const url = `https://firestore.googleapis.com/v1/projects/${firebaseProjectId}/databases/(default)/documents/users/${encodeURIComponent(
      emailKey,
    )}?updateMask.fieldPaths=isPremium&key=${firebaseApiKey}`;
    const response = await fetch(url, {
      method: 'PATCH',
      headers: {
        'Content-Type': 'application/json',
    ...
    console.error(
      '[Stripe Webhook] Exception lors de la mise à jour Firestore REST Premium :',
      err,
    );
  }
}
